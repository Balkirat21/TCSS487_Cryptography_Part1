...
## Practical Project - Part 1: Symmetric Cryptography
...
This project implements a complete SHA-3 and SHAKE cryptographic library in Java, providing symmetric encryption capabilities based on the Keccak sponge construction as specified in FIPS 202.
...
- ✗ MAC computation for direct text input (not implemented)

### Files Submitted

1. `Main.java` - Command-line application interface
2. `SHA3SHAKE.java` - Cryptographic library implementation
3. `REPORT.pdf` - This report
...
1. **SHA3SHAKE** - Core cryptographic library implementing:
...
2. **Main** - Command-line interface providing:
   - File hashing operations
   - MAC generation with passphrases
...
The implementation passes all NIST test vectors and handles all edge cases correctly:
